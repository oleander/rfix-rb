...
module Rfix
  class Repository < Dry::Struct
    ...
    def to_s
      object = Hash.new { |h, k| h[k] = EMPTY_ARRAY.dup }
      files.each_with_object(object) do |file, object|
    ...
    def paths
      permitted.map(&:basename).map(&:to_path).to_a
    end

    def include_file?(path)
      Types::Path::Absolute.call(path).then do |path|
        cache[path.to_s].class.in?(INCLUDED)
      end
    end
